/*
   ...
   UPDATES & MORE DETAILED DOCUMENTATION AT: http://www.Tween.com (there's a link to the AS3 version)
   DESCRIPTION:
   ...
   I created Tween because I needed a very compact tweening engine that was fast and efficient (I couldn't
   ...
   Tween has grown more popular than I could have imagined.

   Since then, I've added new capabilities while trying to keep file size way down (3K). TweenFilterLite extends
   Tween and adds the ability to tween filters including ColorMatrixFilter effects like saturation, contrast,
   brightness, hue, and even colorization but it only adds about 3k to the file size. Same syntax as Tween.
   ...
   on complicated prototype alterations that can cause problems with certain compilers. Tween is simple,
   very fast, and more lightweight than any other popular tweening engine. See an interactive speed comparison
   of various tweening engines at the web site (www.Tween.com).
   ...
   Tween.from() method, these variables would define the BEGINNING values). For example:
   alpha: The alpha (opacity level) that the target object should finish at (or begin at if you're
   using Tween.from()). For example, if the target.alpha is 1 when this script is
   ...
   end up at (or begin at if you're using Tween.from()).
   ...
   MovieClip/SoundChannel to end up at (or begin at if you're using Tween.from()).
   tint : Number - To change a DisplayObject's tint/color, set this to the hex value of the tint you'd like
   to end up at(or begin at if you're using Tween.from()). An example hex value would be 0xFF0000.
   ...
   actually begins, set this to true. By default, it's false which causes Tween.from() to render
   ...
   import gs.Tween;
   Tween.to(clip_mc, 1.5, {alpha:0.5, x:120, volume:0});
   ...
   Tween.to(clip_mc, 5, {alpha:0.5, x:120, ease:Back.easeOut, delay:2, onComplete:onFinishTween, onCompleteParams:[5, clip_mc]});
   ...
   Tween.from(clip_mc, 5, {y:"-100", ease:Elastic.easeOut});
   ...
   Tween.to(mc, 2, {x:"-20"}); it'll move the mc.x to the left 20 pixels which is the same as doing
   Tween.to(mc, 2, {x:mc.x - 20});
   - You can change the Tween.defaultEase function if you prefer something other than Regular.easeOut.
   ...
   Tween.to(myClip_mc, 1.5, {volume:0});
   - You can tween the color of a MovieClip using the tween property "tint", like:
   Tween.to(myClip_mc, 1.5, {tint:0xFF0000});
   - To tween an array, just pass in an array as a property named endArray like:
   var myArray:Array = [1,2,3,4];
   Tween.to(myArray, 1.5, {endArray:[10,20,30,40]});
   - You can kill all tweens for a particular object (usually a MovieClip) anytime with the
   Tween.killTweensOf(myClip_mc); function. If you want to have the tweens forced to completion,
   pass true as the second parameter, like Tween.killTweensOf(myClip_mc, true);
   - You can kill all delayedCalls to a particular function using Tween.killDelayedCallsTo(myFunction_func);
   This can be helpful if you want to preempt a call.
   - Use the Tween.from() method to animate things into place. For example, if you have things set up on
   ...
   6.03:
   - Added the "renderOnStart" property that can force Tween.from() to render only when the tween actually starts (by default, it renders immediately even if the tween has a delay.)
   6.02:
   - Fixed bug that could cause Tween.delayedCall() to generate a 1010 error.
   6.01:
   - Fixed bug that could cause Tween.from() to not render the values immediately.
...
package com.swfgui.effects
{
	...
	public class Tween
	{
		...
		public function initTweenVals($hrp:Boolean = false, $reservedProps:String = ""):void
		{
			...
			else
			{
				for (p in this.vars)
				{
					...
					{ //If we're trying to change the color of a DisplayObject, then set up a quasai proxy using an instance of a Tween to control the color.
					...
					{ //If we're trying to change the volume of a MovieClip or Sound object, then set up a quasai proxy using an instance of a Tween to control the volume.
		...
		public static function delayedCall($delay:Number, $onComplete:Function, $onCompleteParams:Array = null, $onCompleteScope:* = null):Tween
		{
			return new Tween($onComplete, 0, {delay:$delay, onComplete:$onComplete, onCompleteParams:$onCompleteParams, onCompleteScope:$onCompleteScope, overwrite:false}); //NOTE / TO-DO: There may be a bug in the Dictionary class that causes it not to handle references to objects correctly! (I haven't verified this yet)
		}
		...
		public static function removeTween($t:Tween = null):void
		{
