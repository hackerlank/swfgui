package com.swfgui.core
{
	...
	public class DepthQueue
	{
		 ...
		 * 插入一个元素
		 */		
		public function insert(client:IValidatable):void
		{
		 ...
		 * 从队列尾弹出深度最大的一个对象
		 */		
		public function pop():IValidatable
		{
			var client:IValidatable = null;
		 ...
		 * 从队列首弹出深度最小的一个对象
		 */		
		public function shift():IValidatable
		{
			var client:IValidatable = null;
		 ...
		 */
		public function removeLargestChild(client:IValidatable ):Object
		{
			...
			while (min <= max)
			{
				...
				if (bin && bin.length > 0)
				{
					if (max == client.nestLevel)
					{
						if (bin.items[client])
						{
							remove(IValidatable(client), max);
							return client;
						}
					}
					else
		 ...
		 */
		public function removeSmallestChild(client:IValidatable ):Object
		{
			var min:int = client.nestLevel;
			...
			while (min <= maxDepth)
			{
				...
				{   
					if (min == client.nestLevel)
					{
						if (bin.items[client])
						{
							remove(IValidatable(client), min);
							return client;
						}
					}
					else
		 ...
		 * 移除一个元素
		 */
		public function remove(client:IValidatable,level:int=-1):IValidatable
		{
